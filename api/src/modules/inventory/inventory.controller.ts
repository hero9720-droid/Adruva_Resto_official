...
export async function recordMovement(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    const moveRes = await client.query(
      `INSERT INTO stock_movements (outlet_id, ingredient_id, type, quantity, unit_cost_paise, total_cost_paise, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [outlet_id, ingredient_id, type, quantity, unit_cost_paise, total_cost_paise, staff_id]
    );
