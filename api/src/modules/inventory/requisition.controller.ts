...
export async function createRequisition(req: Request, res: Response) {
  ...
  const result = await db.query(
    `INSERT INTO stock_requisitions (chain_id, from_outlet_id, to_outlet_id, items, priority, notes, created_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [chain_id, from_outlet_id, to_outlet_id, JSON.stringify(items), priority, notes, staff_id]
  );

  res.status(201).json({ success: true, data: result.rows[0] });
}

export async function getRequisitions(req: Request, res: Response) {
...
export async function updateRequisitionStatus(req: Request, res: Response) {
  ...
  try {
    ...
    if (current.rowCount === 0) throw new AppError(404, 'Requisition not found', 'NOT_FOUND');
    ...
    if (status === 'received' && requisition.status !== 'received') {
      ...
      for (const item of items) {
        ...
        await client.query(
          'UPDATE ingredients SET current_stock = current_stock - $1 WHERE id = $2 AND outlet_id = $3',
          [item.quantity, item.ingredient_id, requisition.to_outlet_id]
        );
        ...
        await client.query(
          ...
          [item.quantity, ingName, requisition.from_outlet_id]
        );
        ...
        await client.query(
          ...
          [requisition.to_outlet_id, item.ingredient_id, item.quantity, `Requisition #${id} shipped to ${requisition.from_outlet_id}`]
        );
      }
    }
    ...
    const updated = await client.query(
      ...
      [status, staff_id, rejection_reason || null, id]
    );
