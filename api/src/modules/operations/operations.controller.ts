...
export async function createShiftLog(req: Request, res: Response) {
  ...
  const lastShiftRes = await db.query(
    `SELECT closing_cash_paise, created_at 
     FROM shift_logs 
...
export async function getShiftLogs(req: Request, res: Response) {
  const { outlet_id } = req.params;
  const result = await db.query(
    `SELECT sl.*, s.name as manager_name 
     FROM shift_logs sl
...
export async function getHandoverData(req: Request, res: Response) {
  const { outlet_id } = req.params;
...
export async function getHandoverPreview(req: Request, res: Response) {
  const { outlet_id } = req.params;
  ...
  for (const row of txRes.rows) {
    if (row.method === 'cash') cash_sales += Number(row.total);
    if (row.method === 'upi') upi_sales += Number(row.total);
    if (row.method === 'card') card_sales += Number(row.total);
  }
