...
export async function clockIn(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    const check = await client.query(
      `SELECT id FROM attendance WHERE staff_id = $1 AND date = $2 AND clock_out IS NULL`,
      [staff_id, today]
    );
    if ((check.rowCount ?? 0) > 0) throw new AppError(400, 'Already clocked in today', 'ALREADY_CLOCKED_IN');

    const r = await client.query(
      `INSERT INTO attendance (outlet_id, staff_id, date, clock_in)
       VALUES ($1, $2, $3, NOW()) RETURNING *`,
      [outlet_id, staff_id, today]
    );
    return r.rows[0];
  });
...
export async function clockOut(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    if ((r.rowCount ?? 0) === 0) throw new AppError(400, 'Not clocked in', 'NOT_CLOCKED_IN');
    return r.rows[0];
  });
...
export async function getSchedule(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    const r = await client.query(
      `SELECT sh.*, s.name as staff_name, s.role
       FROM shifts sh
       JOIN staff s ON s.id = sh.staff_id
       WHERE s.outlet_id = $1
       ORDER BY sh.day_of_week ASC, sh.start_time ASC`,
      [outlet_id]
    );
    return r.rows;
  });
