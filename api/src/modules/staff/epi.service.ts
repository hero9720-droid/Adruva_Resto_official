...
export async function calculateDailyEPI(outlet_id: string, date: string) {
  // Logic: 
  // 1. Fetch all staff who worked today
  const staffRes = await db.query(`SELECT id, role FROM staff WHERE outlet_id = $1 AND is_active = TRUE`, [outlet_id]);
  
  for (const staff of staffRes.rows) {
    // A. Punctuality Rating
    const attRes = await db.query(`SELECT is_late FROM attendance WHERE staff_id = $1 AND DATE(clock_in) = $2`, [staff.id, date]);
...
export async function getStaffInsights(staff_id: string) {
