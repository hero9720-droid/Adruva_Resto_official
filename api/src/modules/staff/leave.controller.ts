...
export async function requestLeave(req: Request, res: Response) {
  ...
  if (!start_date || !end_date) throw new AppError(400, 'Dates are required', 'VALIDATION_ERROR');

  const result = await withOutletContext(outlet_id, async (client) => {
    const r = await client.query(
      `INSERT INTO leave_requests (outlet_id, staff_id, start_date, end_date, reason, type, status)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending') RETURNING *`,
      [outlet_id, staff_id, start_date, end_date, reason, type || 'sick']
    );
    return r.rows[0];
  });
...
export async function updateLeaveStatus(req: Request, res: Response) {
  ...
  if (!['approved', 'rejected'].includes(status)) throw new AppError(400, 'Invalid status', 'VALIDATION_ERROR');
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    const r = await client.query(
      `UPDATE leave_requests
       ...
       RETURNING *`,
      [status, manager_notes, id, outlet_id]
    );
    if (r.rowCount === 0) throw new AppError(404, 'Leave request not found', 'NOT_FOUND');
    return r.rows[0];
  });
