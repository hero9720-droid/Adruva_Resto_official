...
export async function runPredictiveAnalysis(req: Request, res: Response) {
  const { outlet_id } = req.params;

  // 1. Fetch Sales History (Last 30 Days)
  const history = await db.query(
    `SELECT menu_item_id, name, SUM(quantity) as total_qty
     FROM order_items
  ...
  await db.query('DELETE FROM demand_forecasts WHERE outlet_id = $1 AND predicted_date >= CURRENT_DATE', [outlet_id]);
  
  for (const p of predictions) {
    await db.query(
      `INSERT INTO demand_forecasts (outlet_id, item_type, target_id, predicted_date, predicted_quantity, analysis_basis)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [p.outlet_id, p.item_type, p.target_id, p.predicted_date, p.predicted_quantity, p.analysis_basis]
    );
  }
...
export async function getProcurementPlan(req: Request, res: Response) {
  const { outlet_id } = req.params;
  ...
  const result = await db.query(
    `SELECT 
     ...
     HAVING SUM(df.predicted_quantity * ri.quantity) > i.current_stock`,
    [outlet_id]
  );
...
export async function getForecastSummary(req: Request, res: Response) {
  const { outlet_id } = req.params;
  const result = await db.query(
    `SELECT df.*, mi.name as item_name
     FROM demand_forecasts df
     JOIN menu_items mi ON mi.id = df.target_id
     WHERE df.outlet_id = $1 AND df.predicted_date >= CURRENT_DATE
     ORDER BY df.predicted_date ASC`,
    [outlet_id]
  );
  res.json({ success: true, data: result.rows });
}
