...
export async function evaluateCustomerTier(customer_id: string) {
...
export async function getCustomerPass(customer_id: string) {
...
export async function getChainTiers(chain_id: string) {
  const res = await db.query(`SELECT * FROM loyalty_tier_rules WHERE chain_id = $1 ORDER BY min_spend_paise ASC`, [chain_id]);
  return res.rows;
}
