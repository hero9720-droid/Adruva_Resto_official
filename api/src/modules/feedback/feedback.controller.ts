...
export async function submitFeedback(req: Request, res: Response) {
  ...
  if (!outlet_id || !rating_food) throw new AppError(400, 'Required fields missing', 'VALIDATION_ERROR');
  ...
  if (avgRating >= 4 || posCount > negCount) sentiment = 'positive';
  else if (avgRating <= 2 || negCount > posCount) sentiment = 'negative';
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    const r = await client.query(
      `INSERT INTO customer_feedback (outlet_id, bill_id, customer_id, rating_food, rating_service, rating_ambience, comment, sentiment)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [outlet_id, bill_id || null, customer_id || null, rating_food, rating_service || 3, rating_ambience || 3, comment || '', sentiment]
    );

    // If negative, trigger alert
    if (sentiment === 'negative') {
      const customer = customer_id ? await db.query('SELECT phone FROM customers WHERE id = $1', [customer_id]) : null;
...
export async function getOutletFeedback(req: Request, res: Response) {
