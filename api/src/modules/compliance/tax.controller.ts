...
export async function getTaxSummary(req: Request, res: Response) {
  ...
  if (!start_date || !end_date) throw new AppError(400, 'Start and End dates are required', 'VALIDATION_ERROR');
...
export async function getHSNReport(req: Request, res: Response) {
  const { outlet_id } = req.params;
...
export async function updateHSNCodes(req: Request, res: Response) {
  const { outlet_id } = req.params;
  const { mappings } = req.body; // Array of { item_id, hsn_code }

  if (!Array.isArray(mappings)) throw new AppError(400, 'Invalid mappings', 'VALIDATION_ERROR');

  for (const m of mappings) {
    await db.query('UPDATE menu_items SET hsn_code = $1 WHERE id = $2 AND outlet_id = $3', [m.hsn_code, m.item_id, outlet_id]);
  }
