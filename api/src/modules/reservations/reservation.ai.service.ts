...
export async function predictTableAvailability(outlet_id: string) {
...
export async function processSmartBooking(outlet_id: string, data: any) {
  const { customer_id, party_size, reservation_at } = data;
  
  // 1. Check VIP Status
  const customer = await db.query(`SELECT lifetime_spend_paise FROM customers WHERE id = $1`, [customer_id]);
