  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    if (table_id) {
      ...
      if ((conflict.rowCount ?? 0) > 0) {
        throw new AppError(400, 'Table already booked for this time slot', 'TABLE_CONFLICT');
      }
    }

    // Create reservation (exact DB columns)
    const r = await client.query(
      `INSERT INTO reservations (outlet_id, customer_name, phone, party_size, table_id, reservation_at, notes, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending') RETURNING *`,
      [outlet_id, customer_name, phone, party_size || 2, table_id, reservation_at, notes]
    );
    return r.rows[0];
  });
...
export async function getReservations(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    const params: any[] = [outlet_id, targetDate];
    let query = `
...
export async function updateStatus(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    const r = await client.query(
      'UPDATE reservations SET status = $1 WHERE id = $2 AND outlet_id = $3 RETURNING *',
      [status, id, outlet_id]
    );
    if ((r.rowCount ?? 0) === 0) throw new AppError(404, 'Reservation not found', 'NOT_FOUND');
...
export async function createPublicReservation(req: Request, res: Response) {
  ...
  if (!outlet_id) throw new AppError(400, 'outlet_id is required', 'INVALID_INPUT');

  const result = await withOutletContext(outlet_id, async (client) => {
    // Public reservations usually don't pick a table, staff assigns later.
    // But we record the request.
    const r = await client.query(
      `INSERT INTO reservations (outlet_id, customer_name, phone, party_size, reservation_at, notes, status)
       VALUES ($1, $2, $3, $4, $5, $6, 'confirmed') RETURNING *`,
      [outlet_id, customer_name, phone, party_size || 2, reservation_at, notes]
    );
    return r.rows[0];
  });
