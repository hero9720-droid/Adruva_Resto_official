...
export async function createBooking(req: Request, res: Response) {
  ...
  const settingsRes = await db.query('SELECT * FROM reservation_settings WHERE outlet_id = $1', [outlet_id]);
  ...
  if (settings?.auto_assign) {
    const tableRes = await db.query(
      ...
      [outlet_id, party_size]
    );
    if (tableRes.rowCount && tableRes.rowCount > 0) assignedTableId = tableRes.rows[0].id;
  }
