...
export async function createExpense(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    const r = await client.query(
      `INSERT INTO expenses (outlet_id, category, amount_paise, description, expense_date, receipt_url, submitted_by, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending') RETURNING *`,
      [outlet_id, category, amount_paise, description, expense_date || new Date().toISOString().split('T')[0], receipt_url, staff_id]
    );
    return r.rows[0];
  });
