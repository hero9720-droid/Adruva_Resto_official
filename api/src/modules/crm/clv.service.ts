...
export async function calculateCLVMetrics(chain_id: string) {
  ...
  try {
    ...
    for (const c of customers.rows) {
      ...
      // 3. Segment Logic
      let segment = 'NEW';
      if (monetary > 5000000) segment = 'WHALE'; // > ₹50,000
      else if (frequency > 10) segment = 'LOYAL';
      else if (recency > 90) segment = 'CHURNED';
      else if (recency > 45) segment = 'AT_RISK';
...
export async function getCLVSegments(chain_id: string) {
...
export async function getAtRiskWhales(chain_id: string) {
  const res = await db.query(`
