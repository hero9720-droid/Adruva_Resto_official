...
import { db, withOutletContext } from '../../lib/db';
import { AppError } from '../../lib/errors';
import { getNextOrderNumber } from '../../lib/counters';

export async function ingestExternalOrder(req: Request, res: Response) {
  ...
  const result = await withOutletContext(outlet_id, async (client) => {
    ...
    const orderNumber = await getNextOrderNumber(client, outlet_id);
    ...
    const orderRes = await client.query(
      ...
      ) VALUES ($1, $2, 'confirmed', 'delivery', $3, $4, $5, $6) RETURNING *`,
      [outlet_id, orderNumber, platform?.id || null, external_order_id, commission_paise, total_paise]
    );
    ...
    for (const item of items) {
      await client.query(
        `INSERT INTO order_items (order_id, outlet_id, menu_item_id, name, quantity, price_paise, status)
         VALUES ($1, $2, $3, $4, $5, $6, 'preparing')`,
        [order.id, outlet_id, item.menu_item_id, item.name, item.quantity, item.price_paise]
      );
    }
...
export async function listPlatforms(req: Request, res: Response) {
  const chain_id = req.user.chain_id;
  const result = await db.query('SELECT * FROM delivery_platforms WHERE chain_id = $1', [chain_id]);
  res.json({ success: true, data: result.rows });
}
