...
export async function createModule(req: Request, res: Response) {
  ...
  const result = await db.query(
    ...
    'COMMIT;',
    [chain_id, title, description, content_url, type, min_passing_score || 70, estimated_minutes || 15, JSON.stringify(questions)]
  );
...
export async function getModuleExam(req: Request, res: Response) {
  const { id } = req.params;
  const exam = await db.query('SELECT * FROM training_exams WHERE module_id = $1', [id]);
  if (exam.rowCount === 0) throw new AppError(404, 'Exam not found', 'NOT_FOUND');

  // Strip correct answers for the frontend
  const safeQuestions = exam.rows[0].questions.map((q: any) => ({
    question: q.question,
    options: q.options
  }));
...
export async function submitExam(req: Request, res: Response) {
  ...
  const examRes = await db.query('SELECT questions FROM training_exams WHERE module_id = $1', [id]);
  const moduleRes = await db.query('SELECT min_passing_score FROM training_modules WHERE id = $1', [id]);
  
  if (examRes.rowCount === 0) throw new AppError(404, 'Exam not found', 'NOT_FOUND');
  
  const questions = examRes.rows[0].questions;
  let correctCount = 0;

  questions.forEach((q: any, idx: number) => {
    if (q.correct_index === answers[idx]) correctCount++;
  });
  ...
  if (passed) {
    await db.query(
      `INSERT INTO staff_certifications (staff_id, module_id, score, passed_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (staff_id, module_id) DO UPDATE SET score = EXCLUDED.score, passed_at = NOW()`,
      [staff_id, id, score]
    );
  }
