 ...
 */
export async function checkPlanLimit(outlet_id: string, resource: 'max_tables' | 'max_staff' | 'max_menu_items' | 'max_orders_per_month') {
  ...
  if (limit !== null && parseInt(current_count) >= limit) {
    throw new AppError(422, `Plan limit exceeded for ${resource.replace('max_', '')}. Limit: ${limit}`, 'PLAN_LIMIT_EXCEEDED', {
