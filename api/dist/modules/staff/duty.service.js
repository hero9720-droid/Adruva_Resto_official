...
async function clockIn(staff_id, outlet_id, method = 'manual') {
    const today = new Date().toISOString().split('T')[0];
    // 1. Check if already clocked in
    const existing = await db_1.db.query(`SELECT id FROM attendance WHERE staff_id = $1 AND date = $2`, [staff_id, today]);
    if ((existing.rowCount ?? 0) > 0)
        throw new errors_1.AppError(400, 'Already clocked in for today', 'ALREADY_CLOCKED_IN');
    // 2. Identify the active shift
    const now = new Date().toLocaleTimeString('en-GB', { hour12: false });
    const shiftRes = await db_1.db.query(`
  ...
  `, [outlet_id, now]);
    ...
    if (shift) {
        ...
        if (diffMins > shift.late_buffer_minutes)
            is_late = true;
    }
...
async function clockOut(staff_id, outlet_id, method = 'manual') {
    const today = new Date().toISOString().split('T')[0];
    const result = await db_1.db.query(`
    UPDATE attendance 
  ...
  `, [method, staff_id, outlet_id, today]);
    if ((result.rowCount ?? 0) === 0)
        throw new errors_1.AppError(404, 'No active clock-in found for today', 'NOT_FOUND');
    return result.rows[0];
}
async function getLiveRoster(outlet_id) {
