...
exports.updateCustomerSegments = updateCustomerSegments;
exports.processDailyAutomatedMarketing = processDailyAutomatedMarketing;
const db_1 = require("../../lib/db");
async function getCustomerInsights(chain_id) {
    const segments = await db_1.db.query(`
    SELECT 
      segment, 
      COUNT(*) as count,
    ...
    GROUP BY segment
  `, [chain_id]);
    const retention = await db_1.db.query(`
    SELECT 
...
async function updateCustomerSegments(chain_id) {
...
async function processDailyAutomatedMarketing(chain_id) {
    const today = new Date().toISOString().substring(5, 10); // MM-DD
    // Birthday Rewards
    const birthdays = await db_1.db.query(`
