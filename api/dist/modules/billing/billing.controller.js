...
const loyalty_1 = require("../../lib/loyalty");
async function getBillsList(req, res) {
...
async function generateBill(req, res) {
    ...
    const { order_id, // Switched to single order_id to match schema
    discount_paise = 0, discount_type, service_charge_paise = 0, gst_5_paise = 0, gst_12_paise = 0, gst_18_paise = 0, coupon_code, customer_id, } = req.body;
    if (!order_id) {
        throw new errors_1.AppError(400, 'order_id is required', 'INVALID_INPUT');
    }
    try {
        const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
            // 1. Next bill number
            const bill_number = await (0, counters_1.getNextBillNumber)(client, outlet_id);
            // 2. Subtotal from order items
            const itemsRes = await client.query(`SELECT COALESCE(SUM(total_paise), 0)::bigint AS subtotal FROM order_items WHERE order_id = $1`, [order_id]);
            ...
            // 3. Create bill
            const billRes = await client.query(`INSERT INTO bills (
           ...
           total_paise, customer_id, created_by, status
            ...
            await client.query('UPDATE orders SET status = $1 WHERE id = $2', ['served', order_id]);
            return bill;
        });
        (0, websocket_1.emitToBilling)(outlet_id, 'bill:new', result);
...
async function recordPayment(req, res) {
    ...
    try {
        const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
            ...
            const paymentRes = await client.query(`INSERT INTO payment_transactions (outlet_id, bill_id, method, amount_paise, status)
         VALUES ($1, $2, $3, $4, 'captured') RETURNING *`, [outlet_id, bill_id, payment_method, amount_paise]);
            // 2. Check if bill is fully paid
            const totalPaidRes = await client.query(`SELECT COALESCE(SUM(amount_paise), 0)::bigint AS paid
         FROM payment_transactions
         WHERE bill_id = $1 AND status = 'captured'`, [bill_id]);
            ...
            if (paidAmount >= billTotal) {
                await client.query(`UPDATE bills SET status = 'paid', paid_paise = $1, updated_at = NOW() WHERE id = $2`, [paidAmount, bill_id]);
                ...
                if (customer_id) {
                    ...
                    await (0, loyalty_1.awardLoyaltyPoints)(client, outlet_id, chain_id, customer_id, bill_id, billTotal);
                }
                if (order_id) {
                    // Update table to available
                    await client.query(`UPDATE tables SET status = 'available'
             WHERE id IN (SELECT table_id FROM orders WHERE id = $1)`, [order_id]);
                }
            }
            return paymentRes.rows[0];
        });
        (0, websocket_1.emitToBilling)(outlet_id, 'payment:recorded', result);
        res.json({ success: true, data: result });
    }
    catch (error) {
...
async function getBillDetails(req, res) {
    ...
    const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
        ...
        const ordersRes = await client.query(`SELECT o.*,
