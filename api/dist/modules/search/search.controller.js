...
async function globalSearch(req, res) {
    ...
    const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
        // 1. Search Bills
        const bills = await client.query(`SELECT id, bill_number, total_paise, status, created_at 
       FROM bills 
       WHERE outlet_id = $1 AND (bill_number::text ILIKE $2)
       LIMIT 5`, [outlet_id, searchTerm]);
        // 2. Search Orders
        const orders = await client.query(`SELECT id, order_number, status, total_paise, created_at 
       FROM orders 
       WHERE outlet_id = $1 AND (order_number::text ILIKE $2)
       LIMIT 5`, [outlet_id, searchTerm]);
        // 3. Search Customers
        const customers = await client.query(`SELECT id, name, phone, loyalty_points 
       FROM customers 
