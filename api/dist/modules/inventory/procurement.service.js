...
async function getProcurementSuggestions(outlet_id) {
...
async function generateAutoPOs(outlet_id, staff_id) {
    ...
    for (const [supplierId, items] of Object.entries(bySupplier)) {
        if (supplierId === 'unassigned')
            continue;
        // Create PO
        const poRes = await db_1.db.query(`
    ...
    `, [outlet_id, supplierId, `AUTO-${Date.now().toString().slice(-6)}`, staff_id]);
      ...
      `, [outlet_id, poId, item.id, item.reorder_quantity, item.avg_cost_paise]);
        ...
        await db_1.db.query('UPDATE purchase_orders SET total_amount_paise = $1 WHERE id = $2', [totalAmount, poId]);
        poCount++;
    }
