...
async function recordMovement(req, res) {
    ...
    const { ingredient_id, type, quantity, unit_cost_paise = 0, supplier_id, delivery_rating, delivery_status, delivery_notes } = req.body;
    const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
        const total_cost_paise = unit_cost_paise * quantity;
        const moveRes = await client.query(`INSERT INTO stock_movements (
...
async function getSupplierPerformance(req, res) {
    ...
    const result = await db.query(`SELECT s.*,
     ...
     ORDER BY s.performance_score DESC`, [chain_id]);
...
async function updateSupplier(req, res) {
    ...
    const result = await (0, db_1.withOutletContext)(outlet_id, async (client) => {
        ...
        const r = await client.query(`UPDATE suppliers SET ${setClause}, updated_at = NOW() 
       WHERE id = $${fields.length + 1} AND outlet_id = $${fields.length + 2} RETURNING *`, [...params, id, outlet_id]);
        if (r.rows.length === 0)
            throw new errors_1.AppError(404, 'Supplier not found', 'NOT_FOUND');
        return r.rows[0];
    });
    res.json({ success: true, data: result });
}
async function deleteSupplier(req, res) {
    ...
    await (0, db_1.withOutletContext)(outlet_id, async (client) => {
        const r = await client.query('DELETE FROM suppliers WHERE id = $1 AND outlet_id = $2', [id, outlet_id]);
        if (r.rowCount === 0)
            throw new errors_1.AppError(404, 'Supplier not found', 'NOT_FOUND');
    });
...
async function completeTransfer(req, res) {
    ...
    try {
        ...
        if (stRes.rowCount === 0)
            throw new errors_1.AppError(404, 'Transfer not found', 'NOT_FOUND');
        const transfer = stRes.rows[0];
        if (transfer.status === 'received' || transfer.status === 'cancelled') {
            throw new errors_1.AppError(400, 'Transfer is already finalized', 'ALREADY_FINAL');
        }
        // 2. If status is 'received', perform the stock swap
        if (status === 'received') {
            // Deduct from source
            const deductRes = await client.query('UPDATE ingredients SET current_stock = current_stock - $1 WHERE id = $2 AND outlet_id = $3 RETURNING id', [transfer.quantity, transfer.ingredient_id, transfer.from_outlet_id]);
            if (deductRes.rowCount === 0)
                throw new errors_1.AppError(400, 'Source ingredient not found', 'SOURCE_NOT_FOUND');
            // Add to destination
            ...
            const addRes = await client.query('UPDATE ingredients SET current_stock = current_stock + $1 WHERE id = $2 AND outlet_id = $3 RETURNING id', [transfer.quantity, transfer.ingredient_id, transfer.to_outlet_id]);
            ...
            if (addRes.rowCount === 0) {
                ...
                await client.query('UPDATE ingredients SET current_stock = current_stock + $1 WHERE name = $2 AND outlet_id = $3', [transfer.quantity, sourceIng.rows[0].name, transfer.to_outlet_id]);
            }
        }
        // 3. Update status
        const updated = await client.query('UPDATE stock_transfers SET status = $1, received_by = $2, updated_at = NOW() WHERE id = $3 RETURNING *', [status, staff_id, id]);
...
async function deductStockFromRecipe(client, outlet_id, menu_item_id, quantity = 1) {
  ...
  `, [menu_item_id, outlet_id]);
    for (const item of recipeItems.rows) {
       ...
       SET current_stock = current_stock - $1, 
           updated_at = NOW() 
       WHERE id = $2 AND outlet_id = $3`, [totalDeduct, item.ingredient_id, outlet_id]);
        // 3. Record movement
        await client.query(`INSERT INTO stock_movements (outlet_id, ingredient_id, type, quantity, notes)
       VALUES ($1, $2, 'kitchen_use', $3, $4)`, [outlet_id, item.ingredient_id, totalDeduct, `Auto-deduct for menu_item ${menu_item_id}`]);
    }
}
