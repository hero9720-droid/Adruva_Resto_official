 ...
 */
async function awardLoyaltyPoints(client, outlet_id, chain_id, customer_id, bill_id, amount_paise) {
    if (!customer_id)
        return;
    ...
    const customerRes = await client.query(`UPDATE customers 
     SET loyalty_points = loyalty_points + $1,
         lifetime_spend_paise = lifetime_spend_paise + $2,
         visit_count = visit_count + 1,
         last_visit_at = NOW()
     WHERE id = $3 RETURNING loyalty_points`, [pointsToEarn, amount_paise, customer_id]);
    ...
    ) VALUES ($1, $2, $3, $4, 'earned', $5, $6)`, [customer_id, outlet_id, chain_id, bill_id, pointsToEarn, newBalance]);
    return pointsToEarn;
}
/**
 ...
 */
async function redeemLoyaltyPoints(client, outlet_id, chain_id, customer_id, points_to_redeem) {
    // 1. Verify balance
    const balRes = await client.query('SELECT loyalty_points FROM customers WHERE id = $1', [customer_id]);
    if (!balRes.rows[0] || balRes.rows[0].loyalty_points < points_to_redeem) {
        throw new Error('Insufficient loyalty points');
    }
    // 2. Deduct points
    const updateRes = await client.query('UPDATE customers SET loyalty_points = loyalty_points - $1 WHERE id = $2 RETURNING loyalty_points', [points_to_redeem, customer_id]);
    const newBalance = updateRes.rows[0].loyalty_points;
    // 3. Record transaction
    await client.query(`INSERT INTO loyalty_transactions (
      customer_id, outlet_id, chain_id, type, points, balance_after
    ) VALUES ($1, $2, $3, 'redeemed', $4, $5)`, [customer_id, outlet_id, chain_id, points_to_redeem, newBalance]);
