      ...
      <div className="flex items-center justify-between mb-8 shrink-0">
        ...
        <div className="flex items-center gap-6">
           <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/60 shadow-inner">
             <div className="flex items-center gap-2 px-6 py-2.5 bg-white rounded-xl shadow-sm border border-slate-200/50">
           ...
           <Button className="h-14 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-glow border-none">
          ...
          <AnimatePresence>
            {rooms?.map((room: any) => (
              <motion.div
                ...
                key={room.id}
                className={cn(
                  "group relative p-8 rounded-[2.5rem] border-2 transition-all duration-500 overflow-hidden",
                  room.status === 'available' ? "bg-white border-slate-100 hover:border-emerald-200 hover:shadow-xl" :
                  room.status === 'occupied' ? "bg-indigo-50/30 border-indigo-100 shadow-soft" :
              ...
              >
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Floor {room.floor}</span>
                    <h3 className="text-3xl font-black text-slate-900 tracking-tighter mt-1">{room.name}</h3>
                  </div>
                  <Badge className={cn(
                    ...
                    room.status === 'occupied' ? "bg-indigo-600 text-white shadow-glow" :
                ...
                {room.status === 'occupied' ? (
                  <div className="space-y-6 relative z-10">
                    <div className="flex items-center gap-4 p-4 bg-white/60 backdrop-blur-md rounded-2xl border border-white shadow-sm">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-bold text-slate-900">{room.guest_name}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between px-2">
                       <div className="flex flex-col">
                          <span className="text-[10px] font-black text-slate-400 uppercase">Room Service</span>
                          <span className="font-black text-2xl text-slate-900">₹{(room.accumulated_paise / 100).toLocaleString()}</span>
                       </div>
                       <Button 
                         variant="outline" 
                         size="sm" 
                    ...
                    <div className="pt-6 grid grid-cols-2 gap-3">
                       <Button 
                         className="h-12 rounded-xl bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 font-black text-xs uppercase"
                         onClick={() => handleCheckOut(room)}
                       >
                         CHECK OUT
                       </Button>
                       <Button className="h-12 rounded-xl bg-slate-900 text-white hover:bg-slate-800 font-black text-xs uppercase">
                         BILLING
                       </Button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-8 h-full justify-between">
                    <div className="flex items-center gap-3 text-slate-400">
                       ...
                       <span className="text-sm font-medium">Ready for guest arrival</span>
                    </div>
                    <Button 
                      className="w-full h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-glow border-none uppercase tracking-widest"
        ...
        {selectedRoom && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
            <motion.div
              ...
              className="bg-white border border-slate-100 rounded-[2.5rem] p-10 max-w-lg w-full shadow-2xl relative overflow-hidden"
            >
              ...
              <div className="relative z-10">
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Check In</h2>
                <p className="text-slate-500 font-medium text-lg mb-10 flex items-center gap-2">
                  Assigning guest to <span className="text-indigo-600 font-black">{selectedRoom.name}</span>
                </p>
                ...
                <div className="space-y-8">
                  <div className="space-y-3">
                    ...
                    <input 
                      ...
                      className="w-full h-16 rounded-2xl bg-slate-50 border-none px-6 text-lg font-bold text-slate-900 placeholder:text-slate-300 focus:ring-2 ring-indigo-500/20 transition-all shadow-inner"
                      value={guestName}
                  ...
                  <div className="flex gap-4">
                    <Button 
                      variant="ghost" 
                      className="flex-1 h-16 rounded-2xl font-black uppercase tracking-widest text-slate-400" 
                      onClick={() => { setSelectedRoom(null); setGuestName(''); }}
                    >
                      Cancel
                    </Button>
                    <Button 
                      className="flex-[2] h-16 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-glow border-none uppercase tracking-widest"
