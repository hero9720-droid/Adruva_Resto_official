...
export default function POSPage() {
  ...
  const renderCartPanel = () => {
    ...
    return (
      <div className="flex flex-col h-full bg-card rounded-2xl border border-border shadow-md overflow-hidden relative">
        {/* Header */}
                  ...
                  <div key={item.id} className="flex items-stretch gap-3 bg-background p-3 rounded-2xl border border-border shadow-sm">
                    ...
                    <div className="flex flex-col flex-1 justify-center min-w-0">
                       <div className="flex justify-between items-start mb-0.5">
                          <span className="font-bold text-foreground text-sm leading-tight line-clamp-2 pr-2 truncate">{item.menu_item_name}</span>
                       </div>
  ...
  return (
    <>
                  ...
                  <div className="p-4 flex flex-col flex-1 justify-between bg-card">
                    <div className="mb-3">
                      <div className="flex justify-between items-start gap-2 mb-1">
                        <h3 className="font-bold text-foreground text-sm md:text-base leading-tight line-clamp-2">{item.name}</h3>
                      </div>
                      <p className="text-xs text-muted-foreground font-medium truncate">{item.description || 'Premium selection'}</p>
                    </div>
                    <div className="flex justify-between items-center mt-auto">
                      <p className="text-lg md:text-xl font-black text-foreground tracking-tight">₹{item.base_price_paise / 100}</p>
                      <div className="h-8 w-8 md:h-9 md:w-9 rounded-lg bg-muted flex items-center justify-center text-foreground group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
      ...
      {/* Right Area: Instantly Sticky Advanced Order Panel */}
              ...
              <div className="space-y-6 max-h-[50vh] overflow-y-auto no-scrollbar pb-4">
                {isModLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest animate-pulse">Syncing options...</p>
                  </div>
                ) : (
                  <>
